...
#include <windows.h> // AckisExtensions
#include "ISceneManager.h" // AckisExtensions
...
namespace scene
{
	...
	*/
	...
	{
	public:
	protected: // AckisExtensions
  //! Time when the node shall delete itself. // AckisExtensions
  u32 LifeTime; // AckisExtensions
	public: // AckisExtensions
  //! Sets the time when the node shall delete itself. // AckisExtensions
  /*! // AckisExtensions
    \param lt Time in milli seconds. // AckisExtensions
  */ // AckisExtensions
  virtual void setLifeTime(u32 lt){ // AckisExtensions
    LifeTime = GetTickCount() + lt; // AckisExtensions
  } // AckisExtensions
		void setMaterialFlag(u32 idMBuffer, video::E_MATERIAL_FLAG flag, bool newvalue){ // AckisExtensions
			if(idMBuffer < getMaterialCount()) getMaterial(idMBuffer).setFlag(flag, newvalue); // AckisExtensions
		} // AckisExtensions
		bool getMaterialFlag(u32 idMBuffer, video::E_MATERIAL_FLAG flag){ // AckisExtensions
			if(idMBuffer < getMaterialCount()) return getMaterial(idMBuffer).getFlag(flag); // AckisExtensions
      return false; // AckisExtensions
		} // AckisExtensions
		void setMaterialTexture(u32 idMBuffer, u32 textureLayer, video::ITexture* texture){ // AckisExtensions
			if(textureLayer >= video::MATERIAL_MAX_TEXTURES) return; // AckisExtensions
			if(idMBuffer < getMaterialCount()) getMaterial(idMBuffer).setTexture(textureLayer, texture); // AckisExtensions
		} // AckisExtensions
		video::ITexture* getMaterialTexture(u32 idMBuffer, u32 textureLayer){ // AckisExtensions
			if(textureLayer >= video::MATERIAL_MAX_TEXTURES) return 0; // AckisExtensions
			if(idMBuffer < getMaterialCount()) return getMaterial(idMBuffer).getTexture(textureLayer); // AckisExtensions
		  return 0; // AckisExtensions
		} // AckisExtensions
		void setMaterialType(u32 idMBuffer, video::E_MATERIAL_TYPE newType){ // AckisExtensions
			if(idMBuffer < getMaterialCount()) getMaterial(idMBuffer).MaterialType = newType; // AckisExtensions
		} // AckisExtensions
		video::E_MATERIAL_TYPE getMaterialType(u32 idMBuffer){ // AckisExtensions
			if(idMBuffer < getMaterialCount()) return getMaterial(idMBuffer).MaterialType; // AckisExtensions
			return video::EMT_SOLID; // AckisExtensions
		} // AckisExtensions
...
LifeTime = 0; // AckisExtensions
		...
		*/
		...
		{
		  if(LifeTime) if(GetTickCount() >= LifeTime) SceneManager->addToDeletionQueue(this); // AckisExtensions
			if (IsVisible)
			{
